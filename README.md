...
summarizer.py    -> ترجمه و خلاصه‌سازی ساختاریافته به فارسی (Groq Qwen, رایگان) + فرمت HTML تلگرام
image_generator.py -> ساخت یک عکس مرتبط با موضوع اپیزود (پرامپت با Groq LLM + تولید عکس با Pollinations.ai، هر دو رایگان)
telegram_bot.py  -> ارسال خلاصه به کانال تلگرام (همراه با عکس موضوعی اپیزود؛ در صورت شکست، یکی از عکس‌های عمومی assets/)
...
- **عکس هر پست**: چون هر دو منبع پادکست چینی‌اند، اگر لینک اپیزود اصلی در پیام گذاشته می‌شد،
  پیش‌نمایش خودکار تلگرام عکس/متن چینی صفحه‌ی منبع را نشان می‌داد. به‌جایش، `image_generator.py`
  از خلاصه‌ی فارسی‌شده یک پرامپت تصویری انگلیسی می‌سازد (با همان مدل Groq) و با
  [Pollinations.ai](https://pollinations.ai) (رایگان، بدون نیاز به ثبت‌نام یا کلید) عکسی مرتبط
  با موضوع همان اپیزود می‌سازد. اگر این مرحله به هر دلیلی (قطعی سرویس، خطای شبکه) شکست بخورد،
  خودکار به یکی از عکس‌های عمومی تِم هوش‌مصنوعی در `assets/` برمی‌گردد؛ در هیچ حالتی ارسال پیام
  به‌خاطر شکست تولید عکس متوقف نمی‌شود.
